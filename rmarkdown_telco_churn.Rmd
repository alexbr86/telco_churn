...
Una vez entrenado y viendo que el accuracy, 0.76, en entrenamiento es menor del resultado de H2O de AutoML, se va a comprobar su eficiencia predictora y a hacer la matriz de confusión.
...
Por lo tanto este algoritmo tiene un auc de 0.75 con una sensiblidad y especificidad equilibradas con un 0.75. Detecta muy bien los verdaderos positivos en abandono aunque quizás en falsos positivos sea algo alto.
...
En este caso la mejor configuración del set de entrenamiento para el modelo de GLM es con una profundidad de 3 y 100 arboles. Esto lleva a un auc de 0.786, sigue siendo menor al que ha salido con H2o de 0.84. Ahora va a procederse a predecir con el test y ver la matriz de confusión que tal se comporta.
...
Este modelo da un auc algo mas alto, de 0.77 aunque la sensibilidad es algo más baja ya que hay más falsos negativos que en el anterior modelo. Como se ha comentado, aunque la idea es optimizar todos los indicadores, prima el poder detectar todos los que realmente son positivos.
...
Tras entrenar el modelo con XGBoost se encuentra una combinación de parametros donde como resultante el auc da 0.79 aún menor que el resultado de H20. Se va a proceder a predecir y ver la matriz de confusión para ver como se comporta el modelo.
...
Este algoritmo finalmente tiene un auc de 0.77 aunque tiene una sensibilidad aún peor que el anterior algoritmo ya que como puede observaser en la matriz de confusión los falsos negativos son más altos aún que en los anteriores casos.
...
Tras el entrenamiento el algoritmo de Random Forest da un auc de 0.784 aún inferior al resultado obtenido en H2O. Se a a ver que tal se comporta prediciendo con la parte del test y viendo la matriz de confusión.
...
Tras ver que el modelo de GLMNET con el set de entrenamiento da un auc de 0.764 tampoco supera el modelo de H20 con las parametrizaciones optimizadas, se va a comprobar como se comporta prediciendo y en la matriz de confusión.
...
Este modelo tiene un auc de 0.76 con una sensibilidad del 0.766 la más alta hasta ahora y con una especificidad de 0.759. Es el algoritmo con mayor acierto en verdaderos positivos y el que menos tiene en falsos negativos. Ademas de que sus falsos positivos están bastante equilibrados frente al resto.
...
Tras todas estas pruebas, se ha mostrado la mejor solución o elección en cada caso. Una vez analizado cada algoritmo para las mejores soluciones o elecciones planteadas se ha observado que el algoritmo que mejor se adapta a la casuistica con los resultados y metricas ofrecidos es GLMNET. 

Con este algotimo se ha logrado un auc de 76% con una sensibilidad del 76.6% y una especificidad del 75.9%. Estos datos en general no son malos, es un buen comienzo de modelado. Aún así no supera el auc y la especificidad (84% y 90%) del de H2O que era un GBM. Pero como se mencionaba antes, la sensibilidad o ser capaz de acertar el mayor numero de abandonos sin errar en exceso en los falsos negativos es bastante importante. Ya que de cara a negocio es mucho mejor detectar realmente quienes se van que fallar alguno diciendo que se va y luego no. Y en ese caso este algorito de GLMNET es mejor que el de H2O (76.6% frente a 59%) que sus metricas son altas por acertar quienes no se van.

Por tanto puede entenderse que en ciertos aspectos de eficiencia, el algoritmo de GLMNET es mejor que el de H2O (GBM) y  por consiguiente lo supera.

Las variables más importantes de este algoritmo han sido:

```{r}
plot(varImp(model_glmnet_train))
```

Como puede verse en el gráfico hay variables dummies por lo que habría que valorar tanto la variable en si como en los valores que se fija y en los ausentes. Como se puede ver la variable más importante es la de que tipo de contrato, el servicio de internert y el tipo de pago. Como se ha visto antes en el EDA variables cuales tenian una tendencia según el valor que tuvieran, por lo que viendo el gráfico pueden confirmarse las hipotesis que se lanzaban al inicio y las conclusiones plateadas tras el EDA.
...
En cuanto al modelado para detectarlos, se ha llegado a un primer modelo final con una capacidad de detección del 76% de la cual aciertan quíen se va en un 76,6%. Es un buen comienzo que habría que seguir mejorando, investigando otras técnicas de feature engineering, probando con otros modelos e hiperparametros, quizás cambiando el modelo de predición de 0 o 1 a una probabilidad de ser 1 y por supuesto entrenando los algoritmos con nuevos datos.
