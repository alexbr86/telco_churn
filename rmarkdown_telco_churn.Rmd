...
dataset[num_columns] <- sapply(dataset[num_columns], as.numeric)

```

La variable tenure da la información en meses por lo que no es un valor comparable con el resto de continuas y quizás de problemas. Por lo que para poder conseguir un modelo mas eficiente, se va a agregar los meses en franjas de años y categorizarla en factor. De este modo a la hora de leer la variable también será mucho más sencilla de entender.
```{r}
dataset <- mutate(dataset, tenure_bin = tenure)

dataset$tenure_bin[dataset$tenure_bin >=0 & dataset$tenure_bin <= 12] <- '0-1 year'
dataset$tenure_bin[dataset$tenure_bin > 12 & dataset$tenure_bin <= 24] <- '1-2 years'
dataset$tenure_bin[dataset$tenure_bin > 24 & dataset$tenure_bin <= 36] <- '2-3 years'
dataset$tenure_bin[dataset$tenure_bin > 36 & dataset$tenure_bin <= 48] <- '3-4 years'
dataset$tenure_bin[dataset$tenure_bin > 48 & dataset$tenure_bin <= 60] <- '4-5 years'
dataset$tenure_bin[dataset$tenure_bin > 60 & dataset$tenure_bin <= 72] <- '5-6 years'

dataset$tenure_bin <- as.factor(dataset$tenure_bin)
```


Pasar las variables categoricas a dummies para poder trabajar con ellas en los modelos.
```{r}
dataset_churn_factor <- dataset
...
# Fit model
model <- train(Churn~., data=data_train, trControl=train_control, method="glm")
# Summarise Results
print(model)
```
